...
public class Board {
    ...
    private void addSnakesLadders(Cell[][] cells, int numberOfSnakes, int numberOfLadders) {
        ...
        while (numberOfSnakes > 0) {
            int snakeHead = ThreadLocalRandom.current().nextInt(5, cells.length * cells.length - 1);
            int snakeTail = ThreadLocalRandom.current().nextInt(5, cells.length * cells.length - 1);
